...
import { formatScore, screeningTypeLabels } from "@/lib/labels";
import type { TypeComparison } from "@/lib/stats";
...
export function TypeComparisonChart({ data }: { data: TypeComparison[] }) {
  ...
  const chartData = data
    ...
    .map((d) => ({
      type: d.type,
      label: screeningTypeLabels[d.type],
      score: d.score as number,
      screeningCount: d.screeningCount,
    }));
  ...
  const selected = data.find((d) => d.type === selectedType) ?? null;

  return (
    <div className="flex flex-col gap-3">
      <p className="text-xs text-zinc-500 dark:text-zinc-400">
        Auf einen Balken klicken, um die Filme dahinter zu sehen.
      </p>
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <Bar
              ...
              cursor="pointer"
              onClick={(entry: { payload?: { type: ScreeningType } }) => {
        ...
        <div className="rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-950">
          <div className="flex items-center justify-between">
            <p className="text-sm font-medium text-zinc-950 dark:text-zinc-50">
              {screeningTypeLabels[selected.type]} ({selected.screeningCount})
            </p>
            <button
              type="button"
