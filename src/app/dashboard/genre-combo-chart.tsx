...
import type { GenreComboRanking } from "@/lib/stats";

function formatScoreValue(value: unknown): string {
  return formatScore(Number(value));
}

export function GenreComboChart({ data }: { data: GenreComboRanking[] }) {
  const [selectedLabel, setSelectedLabel] = useState<string | null>(null);

  if (data.length === 0) {
    return (
      <p className="text-sm text-zinc-500 dark:text-zinc-400">
        Noch keine Genre-Kombination mit mindestens 2 Vorstellungen.
      </p>
    );
  }
  ...
  return (
    <div className="flex flex-col gap-3">
      <p className="text-xs text-zinc-500 dark:text-zinc-400">
        Nur Kombinationen mit mind. 2 Vorstellungen. Zum Anzeigen der Filme
        klicken.
      </p>
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <Bar
              ...
              cursor="pointer"
              onClick={(entry: { payload?: GenreComboRanking }) => {
